...
def get_jobs(
    ...
    # Consulta base
    query = (
        db.query(Job).filter(Job.published_at >= limit_date)
        if limit_date
        else db.query(Job)
    )
...
def get_top_companies(limit: int = 5, db: Session = Depends(get_db)):
    ...
    resultados = (
        db.query(Company, func.count(Job.id).label("job_count"))
        .join(Job)
        .group_by(Company.id)
    ...
    for empresa, conteo in resultados:
        ...
        top_companies.append({"company": empresa, "job_count": conteo})
