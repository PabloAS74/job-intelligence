...
def guardar_en_bd(ofertas):
    ...
    try:
        ...
        for oferta in ofertas:
            ...
            if not rol:
                rol = Role(name=oferta.role, normalized_name=oferta.role.lower().replace(" ", "-"))
                db.add(rol)
                db.flush()
            ...
            oferta_existente = db.query(Job).filter_by(
            ...
            if not oferta_existente:
                ...
                nuevo_job = Job(
                    ...
                    source=oferta.source,
                    source_id=oferta.source_id,
                    url=str(oferta.url) # Lo pasamos a string por si Pydantic lo tiene como HttpUrl
                )
                db.add(nuevo_job)
                nuevas_ofertas += 1
